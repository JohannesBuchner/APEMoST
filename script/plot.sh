...
for dir in *_*; do 
	...
	for i in *chain0*.prob.dump 
	do
		target=$(echo $i|sed 's/-chain0.prob.dump//g')
		cat $i > $target.all
	done
	...
	for j in *.all; do 
		...
		rm $j
		echo 'set title "'$dir-$j'";set terminal png; set output "'$j.hist.tab.png'"; set term png size 400, 200; ' plot "$(getlimits $j) "'"'$j.hist.tab'" notitle with boxes;'|gnuplot || die 'plot' 4
