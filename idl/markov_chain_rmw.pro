function markov_chain_rmw, prob_obj, index=index, talkative=talkative, calc_index=calc_index, epsilon=epsilon, a=a, iter=iter
...
alpha = exp(prob_new - prob_old)
if alpha gt 1.0D then alpha=1.0D
stepnew = steps + randomu(seed, n_par)/sqrt(iter)*(alpha - 0.234D)
...
prob_obj->set_probability,prob_new, /last
